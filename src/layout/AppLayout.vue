        ...
        <transition name="layout-menu-container">
            <div class="layout-menu-container" @click="onMenuClick" v-show="isMenuVisible()">
                <div class="layout-menu-logo" @click="$router.push({ path: '/' })">
                    <button class="p-link">
                        <img id="layout-menu-logo" :src="logoWhiteHorizontal" alt="babylon-layout" />
                    </button>
                </div>
...
import { userStyleStorage } from '../constants/BrowserStorageVars';
...
export default {
    ...
    mounted() {        
        let userStyle = localStorage.getItem(userStyleStorage);
        if (!userStyle) {
            const dados = {
                profileMode: 'popup',
                layoutMode: 'static',
            ...
            localStorage.setItem(userStyleStorage, JSON.stringify(dados));     
            userStyle = dados;       
        } else {
            userStyle = JSON.parse(userStyle);
        }
        this.profileMode = userStyle.profileMode;
        this.layoutMode = userStyle.layoutMode;
        this.darkMenu = userStyle.darkMenu;
    },
    watch: {
        $route() {
    ...
    methods: {
        ...
        onLayoutChange(layoutMode) {
            let userStorage = JSON.parse(localStorage.getItem(userStyleStorage));
            userStorage.layoutMode = layoutMode;
            localStorage.setItem(userStyleStorage, JSON.stringify(userStorage));
        ...
        onMenuColorChange(menuColor) {
            let userStorage = JSON.parse(localStorage.getItem(userStyleStorage));
            userStorage.darkMenu = menuColor;
            localStorage.setItem(userStyleStorage, JSON.stringify(userStorage));
            this.darkMenu = menuColor;
        },
        onProfileModeChange(profileMode) {
            let userStorage = JSON.parse(localStorage.getItem(userStyleStorage));
            userStorage.profileMode = profileMode;
            localStorage.setItem(userStyleStorage, JSON.stringify(userStorage));
